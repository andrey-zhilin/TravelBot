...
class query():
    ...
    def choosing_random_route(self,origin_city):
        query=f'SELECT departure_airport_iata, arrival_airport_iata, tp1.name_primary, tp2.name_primary, tp1.city_code, tp2.city_code, c1.name_primary, c2.name_primary ' \
            f'FROM routes r ' \
                f'INNER JOIN transport_point tp1 ON r.departure_airport_iata = tp1.point_id ' \
                    f'INNER JOIN transport_point tp2 ON r.arrival_airport_iata = tp2.point_id ' \
                        f'INNER JOIN city c1 on tp1.city_code = c1.city_id ' \
                            f'INNER JOIN city c2 on tp2.city_code = c2.city_id ' \
